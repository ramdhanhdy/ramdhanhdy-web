      ...
      {children.split(' ').map((word, wIdx) => (
        <span key={wIdx} className="inline-block mr-[0.25em]">
          {word.split('').map((char, cIdx) => (
            <span key={cIdx} className="text-char opacity-20 text-zinc-500 transition-colors duration-100">{char}</span>
          ))}
        </span>
      ))}
    </div>
...
export default function About() {
  ...
  return (
    <div ref={containerRef} className="w-full bg-black relative h-screen overflow-hidden">
      ...
      >
        ...
        */}
        <div ref={contentRef} className="max-w-4xl mx-auto w-full px-6 md:px-12 flex flex-col pt-32 pb-40">
          <div ref={textRef} className="text-2xl md:text-3xl lg:text-[2.5rem] leading-[1.3] font-medium tracking-tight cursor-default">
            {paragraphs.map((p, i) => (
      ...
      <div className="fixed bottom-12 right-12 z-40 text-neon text-sm font-mono tracking-widest uppercase hidden md:flex items-center">
